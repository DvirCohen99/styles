"""
Shoshi Tamam adapter — shoshitamam.co.il
Platform: Shopify
...
log = logging.getLogger("engine.adapters.shoshi_tamam")


class ShoshiTamamAdapter(ShopifyMixin, BaseAdapter):
    ...
    def __init__(self):
        super().__init__(
            min_delay=2.5,
            max_delay=5.5,
            extra_headers={"Referer": "https://www.shoshitamam.co.il/"},
        )
    ...
    def discover_category_urls(self) -> list[str]:
        return [
            ...
            f"{self.BASE_URL}/collections/new",
        ]
    ...
    def parse_product(self, raw: RawProductPayload) -> ParseResult:
        ...
        if result.success and result.product and not result.product.brand:
            result.product.brand = "Shoshi Tamam"
        return result
